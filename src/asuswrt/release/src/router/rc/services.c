...
//	char *ddns_cache;
	...
	if (strcmp(server, "WWW.DYNDNS.ORG")==0)
        ...
        else if (strcmp(server, "custom_script") == 0){
                service = "dyndns_script", asus_ddns = 3;
        }
	...
	if(3 == asus_ddns)
	{
		...
		if(strcmp(service, "dyndns_script") == 0){
			logmessage("start_ddns", "run custom script: %s\n", nvram_safe_get("ddns_custom_scrip"));
			char path[PATH_MAX] ;
			sprintf(path, nvram_safe_get("ddns_custom_scrip"));
			if(f_exists(path)){ 
				eval("sh", path,  wan_ip);
				eval("ddns_updated");
				logmessage("start_ddns", "Info run custom script: %s done.\n", nvram_safe_get("ddns_custom_scrip"));
			}else{
				logmessage("start_ddns", "Error custom script: %s not exist.\n", nvram_safe_get("ddns_custom_scrip"));
			}
		}else{  
		...
		logmessage("start_ddns", "WARN ddns_server_x not set?? now running custom script ddns-start !");
